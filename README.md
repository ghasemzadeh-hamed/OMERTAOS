...
## معرفی
ایون‌اواس (AION-OS) یک «سیستم‌عامل وب برای ایجنت‌ها»ست که ساخت، اجرای بلادرنگ و مقیاس‌پذیرِ سامانه‌های چندعاملی را ساده می‌کند. معماری آن ماژولار است: درگاه ورودی (Gateway) برای API و احراز هویت، کنترل‌پلین (Control Plane) برای ارکستراسیون تسک‌ها و مسیریابی هوشمند به مدل‌های زبانی، ماژول‌های اجرا برای کارهای پرفورمنس، و کنسول Next.js برای مانیتورینگ و مدیریت. هستهٔ AION-OS زمان‌بندی ایجنت‌ها، مدیریت حافظه/کانتکست، صف‌بندی درخواست‌ها، کش و گاردریل‌های ایمنی را فراهم می‌کند و نسبت به مدل/ارائه‌دهندهٔ خاص بی‌طرف است (لوکال یا ابری). نتیجه اینکه تیم‌ها می‌توانند با یک SDK توسعه‌پسند، ایجنت‌های نقش‌محور بسازند، ابزارها (وب، دیتابیس، اجرای کد و …) را ایمن متصل کنند، و گردش‌کارهای پیچیده را با مشاهده‌پذیری و کنترل کامل در مقیاس سازمانی اجرا کنند.
