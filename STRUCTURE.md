...
| `registry/`       | Agent/model metadata, lookup and version ownership          | Model-profile reader is implemented; broader registry lifecycle remains incremental |
...
| `tests/`          | Cross-layer, contract, integration and acceptance tests     | System-level verification; unit tests may be colocated with services                |
| `integrations/`   | Event, observability, provider, OS and third-party adapters | Integration-specific translation; no core orchestration                             |
