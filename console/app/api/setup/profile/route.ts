...
export async function GET() {
  ...
  try {
    ...
    const status =
      error instanceof GatewayProfileError && error.status ? error.status : 502;
    const message =
      error instanceof GatewayProfileError
        ? error.message
        : "Failed to fetch setup profile from gateway";
    const hint =
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const status =
      error instanceof GatewayProfileError && error.status ? error.status : 502;
    const message =
      error instanceof Error ? error.message : "Failed to update setup profile";
    const hint =
      process.env.NODE_ENV !== "production"
