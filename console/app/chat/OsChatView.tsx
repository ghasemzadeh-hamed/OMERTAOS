...
export function OsChatView({ initialThreadId }: { initialThreadId?: string }) {
  ...
  useEffect(() => {
    fetchThreads()
      .then((data) => {
        setThreads(data);
        setSelectedThreadId((current) => current ?? initialThreadId ?? data[0]?.id);
      })
  ...
  const handleStop = async () => {
    if (!selectedThreadId) return;
    try {
      await stopThreadActions(selectedThreadId);
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Failed to stop actions';
      setError(message);
    }
  };
        ...
        <div className="flex flex-1 flex-col">
          <div className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3">
            <div>
              ...
              <h1 className="text-lg font-semibold text-slate-800">{activeThread?.title ?? 'Select a thread'}</h1>
            </div>
            <div className="flex items-center gap-2 text-sm text-slate-500">
              <Button variant="outline" size="sm" onClick={refreshMessages}>
                Refresh
              </Button>
              <Button variant="ghost" size="sm" onClick={handleStop} disabled={!selectedThreadId}>
                Stop
              </Button>
            </div>
          </div>
