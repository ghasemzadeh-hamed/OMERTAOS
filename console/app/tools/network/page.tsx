...
import {
  ...
  type ProxyFormState,
  type ProxyProfile,
} from "@/lib/network/proxyValidation";
import {
  CheckCircle2,
  FlaskConical,
  Pencil,
...
const PROXY_API = "/api/system/network/proxies";
...
async function gatewayRequest(path: string, init?: RequestInit) {
  const response = await fetch(`${PROXY_API}${path}`, {
    ...init,
    credentials: "include",
    headers: {
...
export default function NetworkConfigPage() {
  ...
  useEffect(() => {
    load().catch((error) =>
      setStatus(`Failed to load proxy profiles: ${error.message}`),
    );
  }, []);
  ...
  const submit = async () => {
    const validation = validateProxyForm(
      form,
      Boolean(editingProfile?.has_secrets),
    );
    if (validation) {
  ...
  const runProfileAction = async (
    ...
    action: "enable" | "disable" | "test" | "set-default",
  ) => {
    setBusyId(profile.id);
    setStatus(null);
    try {
      const result = await gatewayRequest(`/${profile.id}/${action}`, {
        method: "POST",
        body: JSON.stringify({}),
      });
      await load();
      setStatus(
        action === "test"
          ? `Test ${result.ok ? "passed" : "failed"} via ${result.routed_via}`
  ...
  const remove = async (profile: ProxyProfile) => {
    if (!window.confirm(`Delete proxy profile "${profile.name}"?`)) {
      return;
    }
    setBusyId(profile.id);
    try {
      await gatewayRequest(`/${profile.id}`, { method: "DELETE" });
      await load();
    ...
    <div className="space-y-6 text-right">
      <header className="space-y-1">
        ...
        <p className="text-xs text-white/60">
          Configure outbound routes for external providers through the Gateway
          and Control Plane.
        </p>
      </header>
      ...
      <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_24rem]">
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="space-y-2 text-left">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="rounded bg-white/10 px-2 py-1 text-xs uppercase tracking-normal text-white/70">
                        {profile.type}
                      </span>
                      <span className="rounded bg-cyan-500/15 px-2 py-1 text-xs text-cyan-100">
                        {profile.scope}
                      </span>
                      {profile.is_default && (
                    ...
                    <div className="text-xs text-white/60">
                      ...
                      {profile.health_check_url
                        ? ` / health ${profile.health_check_url}`
                        : ""}
                    </div>
                  </div>
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Button
                      ...
                      disabled={busyId === profile.id}
                      onClick={() => runProfileAction(profile, "test")}
                    >
                    ...
                    <Button
                      ...
                      title="Set default"
                      disabled={busyId === profile.id}
                      onClick={() => runProfileAction(profile, "set-default")}
                    >
        ...
        <form
          className="space-y-4 rounded-md border border-white/10 bg-white/5 p-4 text-left"
          onSubmit={(event) => {
        ...
        >
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-semibold text-white">
              {editingProfile ? "Edit proxy profile" : "Create proxy profile"}
            </h3>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="block space-y-1 text-xs text-white/60">
              ...
              <select
                ...
                value={form.type}
                onChange={(event) =>
                  update({ type: event.target.value as ProxyType })
                }
                className="h-10 w-full rounded-md border border-white/20 bg-slate-950 px-3 text-sm text-white"
              >
            ...
            <label className="block space-y-1 text-xs text-white/60">
              <span>Scope</span>
              <select
                aria-label="Proxy scope"
                value={form.scope}
                onChange={(event) =>
                  update({ scope: event.target.value as ProxyScope })
                }
                className="h-10 w-full rounded-md border border-white/20 bg-slate-950 px-3 text-sm text-white"
              >
                {SCOPES.map((scope) => (
          ...
          <Input
            ...
            onChange={(event) =>
              update({ health_check_url: event.target.value })
            }
          />
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {SECRET_FIELDS.map((field) => (
              <label
                key={field}
                className="block space-y-1 text-xs text-white/60"
              >
                <span>{field}</span>
                <Input
          ...
          <div className="flex items-center justify-between rounded-md border border-white/10 px-3 py-2 text-sm text-white/70">
            ...
            <Switch
              checked={form.fallback_direct}
              onCheckedChange={(checked) =>
                update({ fallback_direct: checked })
              }
            />
          </div>
          <div className="flex items-center justify-between rounded-md border border-white/10 px-3 py-2 text-sm text-white/70">
