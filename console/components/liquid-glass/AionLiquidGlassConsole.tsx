...
const surfaceGroups: ReadonlyArray<{
  label: string;
  items: ReadonlyArray<Surface>;
}> = [
  {
    label: "AI & workflows",
    items: [
      {
        href: "/chat",
        label: "Chat",
        description: "Create and continue assistant threads.",
        icon: MessageSquare,
        minRole: "USER",
      },
      {
        href: "/agent-chat",
        label: "Agent chat",
        description: "Submit focused agent tasks through the Gateway.",
        icon: Workflow,
        minRole: "USER",
      ...
      },
      {
        href: "/agent",
        label: "Agent mode",
        description: "Submit a goal through the Gateway task boundary.",
        icon: Bot,
        minRole: "USER",
        availability: "limited",
      },
      {
        ...
        label: "Tasks",
        description:
          "Task listing requires a Gateway endpoint not currently exposed.",
        icon: ListTodo,
        minRole: "USER",
      ...
      },
      {
        ...
        label: "My agents",
        description: "Agent instance APIs are not currently exposed.",
        icon: Users,
        minRole: "USER",
        availability: "unavailable",
      },
      {
        href: "/agents/catalog",
        label: "Agent catalog",
        description: "Agent catalog APIs are not currently exposed.",
        icon: PackageSearch,
        minRole: "USER",
        availability: "unavailable",
      },
    ...
    items: [
      {
        ...
        label: "System health",
        description: "Inspect live dependency and service health.",
        icon: ShieldCheck,
        minRole: "USER",
      },
      {
        ...
        description: "Host resource metrics are not currently exposed.",
        icon: Activity,
        minRole: "MANAGER",
        availability: "unavailable",
      },
      {
        ...
        description: "Service lifecycle APIs are not currently exposed.",
        icon: Server,
        minRole: "ADMIN",
        availability: "unavailable",
      },
      {
        ...
        label: "Log center",
        description: "Log collection APIs are not currently exposed.",
        icon: FileText,
        minRole: "MANAGER",
        availability: "unavailable",
      },
      {
        href: "/tools/update",
        label: "Update center",
        description: "Signed update APIs are not currently exposed.",
        icon: RefreshCcw,
        minRole: "ADMIN",
        availability: "unavailable",
      },
      {
        ...
        description: "Backup execution APIs are not currently exposed.",
        icon: Archive,
        minRole: "ADMIN",
        availability: "unavailable",
      },
      {
        href: "/tools/metrics",
        label: "Metrics",
        description: "Metrics APIs are not currently exposed.",
        icon: Gauge,
        minRole: "MANAGER",
        availability: "unavailable",
      },
    ...
    items: [
      {
        ...
        label: "Config center",
        description: "Manage runtime paths, profiles, and policy.",
        icon: Settings,
        minRole: "ADMIN",
      },
      {
      ...
      },
      {
        href: "/tools/datasets",
        label: "Datasets",
        description: "Dataset ingestion APIs are not currently exposed.",
        icon: Database,
        minRole: "MANAGER",
        availability: "unavailable",
      },
      {
        ...
        label: "Data studio",
        description: "Data preview APIs are not currently exposed.",
        icon: Boxes,
        minRole: "MANAGER",
        availability: "unavailable",
      },
      {
        href: "/tools/network",
        label: "Network",
        description: "Manage endpoints and TLS configuration.",
        icon: Plug,
        minRole: "ADMIN",
      },
      {
        ...
        label: "File explorer",
        description: "Sandboxed file APIs are not currently exposed.",
        icon: HardDrive,
        minRole: "MANAGER",
        availability: "unavailable",
      },
      {
        href: "/tools/editor",
        label: "Policy editor",
        description: "Runtime-backed file editing is not currently exposed.",
        icon: TerminalSquare,
        minRole: "MANAGER",
        availability: "unavailable",
      },
      {
        href: "/tools/auth",
        label: "Auth & roles",
        description: "RBAC administration APIs are not currently exposed.",
        icon: KeyRound,
        minRole: "ADMIN",
        availability: "unavailable",
      },
    ...
    items: [
      {
        ...
        description: "Tool discovery APIs are not currently exposed.",
        icon: Search,
        minRole: "USER",
        availability: "unavailable",
      },
      {
        ...
        description: "Inspect Claude Code and plugin setup.",
        icon: Cable,
        minRole: "MANAGER",
      },
      {
        href: "/dashboard/feature-catalog",
        label: "Feature catalog",
        description: "Review available product capabilities.",
      ...
      },
      {
        ...
        description: "Manage administrative configuration.",
        icon: Settings,
        minRole: "ADMIN",
      },
      {
        href: "/admin/tenancy",
        label: "Tenancy",
        description: "Tenant administration APIs are not currently exposed.",
        icon: Users,
        minRole: "ADMIN",
        availability: "unavailable",
      },
      {
        href: "/integrations/windows-bridge",
        label: "Windows bridge",
        description: "Review the current Windows/WSL integration design.",
        icon: Cable,
        minRole: "ADMIN",
        availability: "limited",
      },
...
type ServiceState =
  "ok" | "healthy" | "degraded" | "down" | "error" | "unknown";
...
function normalizeRole(role?: string | null): ConsoleRole {
  ...
  return normalized && normalized in rolePriority
    ? (normalized as ConsoleRole)
    : "USER";
}
...
export default function AionLiquidGlassConsole(): JSX.Element {
  ...
  useEffect(() => {
    ...
    async function loadStatus() {
      ...
      try {
        const res = await fetch("/api/system/admin/config/status", {
          cache: "no-store",
        });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data = await res.json();
            ...
            {primaryNav.map((item) => {
              ...
              return (
                <Button
                  key={item.href}
                  asChild
                  variant={item.href === "/console" ? "default" : "ghost"}
            ...
            <div className="mt-2 min-h-0 flex-1 overflow-y-auto border-t border-white/10 pt-3 pr-1">
              {surfaceGroups.map((group) => (
                <section key={group.label} className="mb-4">
                  <h2 className="px-2 pb-1 text-xs font-semibold uppercase text-white/50">
                    {group.label}
                  </h2>
                  <nav className="space-y-0.5" aria-label={group.label}>
                    {group.items.map((item) => {
                      const Icon = item.icon;
                      const permitted =
                        rolePriority[currentRole] >= rolePriority[item.minRole];
                      return (
                        <Link
                          key={item.href}
                          href={item.href}
            ...
            <Card className={`${glass} rounded-xl`}>
              <CardContent className="space-y-3 p-3">
                <div className="flex items-center justify-between">
                  ...
                  <Badge className={statusTone(overallStatus)}>
                    {overallStatus}
                  </Badge>
                </div>
                <div className="flex flex-wrap gap-1">
                  {Object.entries(services).length ? (
                    ...
                    <span className="text-xs text-white/55">
                      {healthError ?? "Loading service health..."}
                    </span>
                  )}
                </div>
              </CardContent>
          ...
          <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
            <Card className={`${glass} rounded-2xl xl:col-span-2`}>
              ...
              <CardContent className="grid grid-cols-1 gap-3 md:grid-cols-3">
                <Stat
                  title="Gateway"
                  value={services.gateway?.status ?? "unknown"}
                  sub={
                    services.gateway?.details ??
                    healthError ??
                    "Waiting for health API"
                  }
                />
                <Stat
                  title="Control"
                  value={services.control?.status ?? "unknown"}
                  sub={
                    services.control?.details ?? "Reported by Gateway health"
                  }
                />
                <Stat
                  title="Console"
                  value={services.console?.status ?? "unknown"}
                  sub={services.console?.details ?? "Local health endpoint"}
                />
              </CardContent>
            </Card>
            ...
            <CardContent className="space-y-6">
              {surfaceGroups.map((group) => (
                <section key={group.label}>
                  <h2 className="mb-2 text-sm font-semibold uppercase text-white/55">
                    {group.label}
                  </h2>
                    ...
                    {group.items.map((item) => {
                      ...
                      return (
                        <Link
                          key={item.href}
                          href={item.href}
                          className="flex min-h-20 items-start gap-3 rounded-md border border-white/10 bg-white/5 px-3 py-3 hover:bg-white/10"
                        >
...
function ActionLink({ href, icon: Icon, label }: ActionLinkProps) {
  return (
    <Button
      asChild
      variant="secondary"
      className="h-12 justify-start bg-white/15 hover:bg-white/25"
    >
      <Link href={href}>
