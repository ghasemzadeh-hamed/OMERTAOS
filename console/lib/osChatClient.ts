...
async function request<T>(url: string, init?: RequestInit): Promise<ApiEnvelope<T>> {
  ...
  const data = (await res.json().catch(() => ({ ok: false, data: null }))) as ApiEnvelope<T>;
  if (!data.ok) {
...
export async function fetchMessages(threadId: string): Promise<ChatMessage[]> {
  const { data } = await request<ChatMessage[]>(`/api/chat/threads/${threadId}/messages`);
  return data;
}

export async function sendMessage(threadId: string, contentText: string): Promise<ChatMessage> {
  const { data } = await request<ChatMessage>(`/api/chat/threads/${threadId}/messages`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function stopThreadActions(threadId: string): Promise<boolean> {
  const { data } = await request<{ stopped: boolean }>(`/api/chat/threads/${threadId}/actions/stop`, {
    method: 'POST',
  });
  return data.stopped;
}
