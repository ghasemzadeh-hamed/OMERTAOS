...
- Services and ports (docker-compose.yml): Control HTTP 8000 and gRPC 50051; Gateway 8080; Console 3000. Dependencies: Postgres 5432, Redis 6379, Qdrant 6333, MinIO 9000/9001.
...
- Environment defaults (dev.env + compose): `AION_CONTROL_BASE_URL` http://control:8000, `AION_CONTROL_GRPC` http://control:50051, `AION_GATEWAY_PORT` 8080, `CONSOLE_PORT` 3000, `NEXT_PUBLIC_GATEWAY_URL` http://gateway:8080 inside containers, and public defaults of `CONTROL_BASE_URL=http://localhost:8000`, `GATEWAY_BASE_URL=http://localhost:8080`, `CONSOLE_BASE_URL=http://localhost:3000`.
- Console to Gateway routing: Console uses `NEXT_PUBLIC_GATEWAY_URL` (default http://gateway:8080) and admin token variables; Gateway reaches Control via `AION_CONTROL_BASE_URL` and `AION_CONTROL_GRPC` (default http://control:8000 and http://control:50051).
...
- Health checks: scripts/smoke.sh polls `http://localhost:8000/healthz`, `http://localhost:8080/health`, and `http://localhost:3000/health` with retries and diagnostics.
...
| Ports & services | Control 8000/50051, Gateway 8080, Console 3000 | Same compose defaults and logging | ✔ |
...
| Dashboard access | Console at http://localhost:3000 via Gateway at http://localhost:8080 | Same URLs documented and logged | ✔ |
