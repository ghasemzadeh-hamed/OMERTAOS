...
| The allowlisted `runtime.echo.v1` path can execute through the authenticated canonical service path on one local Runtime worker. | E2 | R5 commits `711395a`, `681ff8f`; R6.9 commit `cc52204`; `docs/capo/acceptance-report.md` | Constrained live runs demonstrated selection, execution, authenticated Console API -> Gateway -> Control -> Runtime traversal, tenant/correlation/trace/idempotency propagation, Runtime audit fields, same-request replay, and fail-closed post-restart replay. Browser UI traversal, automatic worker registration, multiple workers, durable Control audit export, production isolation, and broader commands remain unproven. |
