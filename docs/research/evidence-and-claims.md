...
| Control persists a tenant-scoped Runtime scheduling and dispatch trail. | E2 | R6.12 commit `c5a9246`; `control/audit/`; Control audit/migration/dispatch tests; `docs/capo/acceptance-report.md` | A constrained live task produced durable schedule/start/success events with actor, task, attempt, node, correlation, trace, outcome, and retry metadata; tenant mismatch returned no events and unauthenticated access returned 403. Events survived PostgreSQL and Control restarts. The table is not cryptographically tamper-evident, independently exported, retention-managed, or evidence of production authorization. |
