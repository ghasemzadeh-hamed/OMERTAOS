...
- **Updates**: `aion-firstboot` runs `apt-get update` and `apt-get upgrade` with unattended retries.
...
- **Logging**: Installer and first-boot logs live in `/var/log/aion-installer.log` and `/var/log/aion-firstboot.log`.

## Customisation

1. Place overrides in `/etc/aion/hardening.d/*.conf`.
