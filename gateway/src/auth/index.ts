...
export const authPreHandler = (requiredRoles: string[] = []) => {
  return async (request: FastifyRequest, _reply: FastifyReply) => {
    ...
    if (gatewayConfig.tenancyMode === 'multi' && gatewayConfig.requireTenantHeader && !context.user?.tenant) {
      throw createError(400, 'Tenant header required in multi-tenant mode');
    }
