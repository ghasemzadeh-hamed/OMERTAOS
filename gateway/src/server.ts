...
import { registerInboundWebhook } from './webhooks/inbound.js';
...
const authGuard = authPreHandler(['user', 'manager', 'admin']);
const streamEmitter = new EventEmitter();
const chatSessions = new Map<string, { events: Array<{ type: string; payload: unknown }> }>();
...
app.post<{ Body: TaskRequest }>('/v1/tasks', { preHandler: [authGuard, rateLimitMiddleware] }, async (request, reply) => {
  ...
  await deliverEvent('task.status.changed', { taskId: result.taskId, status: result.status, intent: result.intent });
...
app.get('/v1/tasks/:id', { preHandler: [authGuard, rateLimitMiddleware] }, async (request, reply) => {
...
app.get('/v1/stream/:id', { preHandler: [authGuard, rateLimitMiddleware] }, async (request, reply) => {
  const { id } = request.params as { id: string };
...
app.post('/v1/chat/execute', { preHandler: [authPreHandler(['manager', 'admin']), rateLimitMiddleware] }, async (request, reply) => {
  const { command } = request.body as { command?: string };
  if (!command) {
    throw reply.badRequest('command is required');
  }
  const execution = await orchestrateCommand(command, {
    actor: request.context?.user?.id ?? 'unknown',
    tenant: request.headers['x-tenant']?.toString(),
  });
  chatSessions.set(execution.sessionId, { events: execution.events.map((event) => ({ type: event.type, payload: event.data })) });
  await deliverEvent('chat.session.created', { sessionId: execution.sessionId, actor: request.context?.user?.id ?? 'unknown' });
  return execution;
});

app.get('/v1/chat/stream/:sessionId', { preHandler: [authGuard, rateLimitMiddleware] }, async (request, reply) => {
  ...
  if (!session) {
    reply.sse({ event: 'error', data: JSON.stringify({ message: 'session not found' }) });
    return;
  }
  for (const event of session.events) {
    reply.sse({ event: event.type, data: JSON.stringify(event.payload) });
  }
});
...
app.register(async (instance) => {
  instance.get(
    ...
    {
      websocket: true,
      preHandler: [authGuard, rateLimitMiddleware],
    },
    (connection, request) => {
